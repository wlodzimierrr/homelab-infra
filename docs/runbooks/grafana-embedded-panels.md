...
5. Simulate embed failure (for example invalid panel template) and verify:
...
- Panels remain in fallback state:
  ...
  - verify dashboard UID/panel IDs in templates
  - verify Grafana auth/session allows iframe access
- Deep link opens wrong scope:
  - check `{serviceId}` and `{timeRange}` placeholders in dashboard path template
